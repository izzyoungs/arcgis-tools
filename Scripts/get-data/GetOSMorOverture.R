...
tool_exec <- function(in_params, out_params) {
  ...
  if (any(data_source %in% c("OSM", "Both"))) {
    ...
    if ("Amenities" %in% selected_type) {
      ...
      osm_amenties_clean <- osm_amenities$osm_points |>
        select(name, geometry) |>
        mutate(tag = "amenity") |>
        st_transform(4326)

      osm_tourism <- osm_query |>
        add_osm_feature(key = "tourism") |>
        osmdata_sf()

      osm_tourism_clean <- osm_tourism$osm_points |>
        select(name, geometry) |>
        mutate(tag = "tourism") |>
        st_transform(4326)
      ...
      osm_shops_clean <- osm_shops$osm_points |>
        select(name, geometry) |>
        mutate(tag = "shop") |>
        st_transform(4326)

      osm_amentities_sf <- rbind(
        osm_amenties_clean,
        osm_tourism_clean,
        osm_shops_clean
      ) |>
  ...
  if (any(data_source %in% c("Overture", "Both"))) {
    ...
    if ("Amenities" %in% selected_type) {
      ...
      suppressWarnings({
        overture_amentiy <- open_dataset("s3://overturemaps-us-west-2/release/2026-04-15.0/theme=places/type=place") |>
          suppressMessages()

        overture_amenity_filtered <- overture_amentiy |>
          filter(
            bbox$xmin > bbox_overture[1],
            bbox$ymin > bbox_overture[2],
            bbox$xmax < bbox_overture[3],
            bbox$ymax < bbox_overture[4],
          ) |>
          collect() |>
          mutate(
            basic_category = pluck(basic_category),
            category = pluck(categories, 1),
            name = pluck(names, 1)
          ) |>
          select(id, geometry, basic_category, category, name, confidence) |>
          filter(confidence > 0.6)

        overture_amentiy_sf <- st_as_sf(overture_amenity_filtered, crs = 4326)
      })
