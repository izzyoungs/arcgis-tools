...
* *Get Replica Network Volumes Equity Areas* - This tool takes in a study area feature class and optionally an equity areas feature class as well as the email used to authenticate to [Replica](https://replicahq.com) trip tables in BigQuery. It returns loaded network volumes (not including home trips or trips by students) that start or end within the study area boundary. If equity areas are returned, it only returns loaded volumes for population who live within the defined equity areas. 